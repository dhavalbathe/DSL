...
def Minimum(listofmarks):
    for i in range(len(listofmarks)):
        if listofmarks[i] != -999:
            Min = listofmarks[i]
            break
    for i in range(listofmarks):
        if listofmarks[i] < Min and listofmarks[i] != -999:
            Min = listofmarks[i]
...
# main function
marksinFDS = []
numberofstudents = int(input("Enter the total number of student: "))
...
while flag == 1:
    ...
    if ch==1:
        ...
        print("Highest frequency is of marks {0} that is {1}".format(marks, fr))
        a = input("Do you want to continue (yes/no): ")
        if a == "yes":
            flag = 1
        else:
